    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div class="bg-white border border-[#E4E7EC] rounded-xl p-5">
            <div class="flex items-center justify-between mb-3">
                ...
                <form method="POST" action="{{ route('business.tax.update-skema', $business) }}">
                    @csrf @method('PUT')
                    <select name="skema_pajak" onchange="this.form.submit()" class="text-xs rounded-lg border-[#D0D5DD] focus:border-[#465FFF] focus:ring-[#465FFF]">
                        <option value="umkm_final" @selected($business->skema_pajak === 'umkm_final')>UMKM Final (0,5%)</option>
                        <option value="badan_normal" @selected($business->skema_pajak === 'badan_normal')>Badan Normal (22%)</option>
                    </select>
                </form>
            </div>
