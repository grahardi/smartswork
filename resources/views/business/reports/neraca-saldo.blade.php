<x-business-layout :business="$business">
    ...
    <div class="px-4 py-5">
        <form method="GET" class="mb-4">
            <label class="text-xs text-[#667085]">Sampai tanggal</label>
        ...
        <div class="bg-white border border-[#E4E7EC] rounded-xl overflow-hidden">
            <table class="w-full text-xs">
                ...
                <tbody>
                    @foreach ($accounts as $row)
                        <tr class="border-t border-[#E4E7EC]">
                            <td class="px-3 py-2 text-[#101828]">{{ $row['account']->kode }} {{ $row['account']->nama }}</td>
                ...
                <tfoot class="bg-[#F9FAFB] font-semibold">
        ...
        @if (round($totalDebit, 2) !== round($totalKredit, 2))
            <p class="text-xs text-[#D92D20] mt-2">⚠️ Total debit dan kredit tidak seimbang — ada kemungkinan data jurnal bermasalah.</p>
        @endif
    </div>
</x-business-layout>
